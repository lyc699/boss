...
<template>
    <div id="content">
        ...
        <el-table :data="tableData">
            ...
            <el-table-column align="center" prop="userNo" label="处理人编号"></el-table-column>
            ...
            <el-table-column align="center" label="操作"  fixed="right" :show-overflow-tooltip="true" min-width='150px'>
                <template slot-scope="scope">
                    <router-link :to="{path:'/approveDetail',query:{loanNo:scope.row.loanNo}}" class="link">查看</router-link>
                </template>
            </el-table-column>
...
export default {
    ...
    methods: {
        ...
        getQuery(isIntPage){
            ...
            this.$axios
                .post("/api/bycx-appr-service/cApApprPushRecord/getApprovePushRecordList", this.query)
                .then(res => {
        ...
        dateChange(val){
            if(val){
                this.query.params.startApplyDate = val[0]
                this.query.params.endApplyDate = val[1]
            }else{
                this.query.params.startApplyDate = ''
                this.query.params.endApplyDate = ''              
            }
        },
        resetForm() {
            this.initData()
