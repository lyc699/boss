...
<template>
    <div>
        ...
        <el-dialog class="autoDialog" :title="title" :visible.sync="addActView" width="35%">
            <el-form ref="ruleForm" :model="ruleForm" :rules="rules" label-width="115px">
                ...
                <el-col :span="24">
                	<el-form-item label="机构/个人名称" prop="name">
                        <el-input v-model="ruleForm.name" placeholder="机构/个人名称" clearable v-if="ruleForm.userType == '38200001'"></el-input>
                        <el-select placeholder="请选择" v-model="ruleForm.name" @change="nameChange" @focus="selectName" v-else>
...
//          date:'',
    ...
    computed:{
        ...
        isDel(){
            return getBtnAuth("syst:aSysSsqAccount:deleteById")
        },
        isEdit(){
            return getBtnAuth("syst:apply:CA.int")
        }
    ...
    methods: {
        ...
        typeChange() {
        	this.ruleForm.name = ''
            this.$axios.post("/api/bycx-contract-service/aSysOfficialSeal/getList", {params:{
                userType:this.ruleForm.userType,
                elecSignType:'30700001'
            }})
            .then(res => {
                if (res.data.code == "0000") {
        ...
        del(scope){
            this.$confirm(`确认删除该帐户？`, '提示', {
                ...
                this.$axios.post('/api/bycx-contract-service/aSysSsqAccount/deleteById',{params: {
                    id:scope.row.id
                }}).then(res => {
                    if (res.data.code == "0000") {
